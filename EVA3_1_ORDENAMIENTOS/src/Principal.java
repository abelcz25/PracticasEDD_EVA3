...
public class Principal {
    ...
    private static int busquedaBinRec(int [] arreglo, int val,int lo, int hi){
        if (lo <= hi){
            int mid = lo + ((hi-lo)/2);
            ...
            if (val == arreglo[mid]){
                ...
                return busquedaBinRec(arreglo, val, mid + 1, hi);
            }else{//menor
                return busquedaBinRec(arreglo, val, lo, mid - 1); 
            }
        }else 
            return -1;
    }
